...
import { B, serif, sans, COURSES, FEED } from '../lib/data.js'
...
export default function CourseDetail() {
  ...
  const feed = FEED.filter(f => f.course===course.name)
  const show = feed.length ? feed : FEED.slice(0,3)
    ...
    <div style={{ maxWidth:680, margin:'0 auto' }}>
      ...
      <div style={{ background:course.bg, borderRadius:20, overflow:'hidden', marginBottom:20 }}>
        ...
        <div style={{ background:'rgba(0,0,0,0.25)', padding:'16px 26px', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
          {[['Overall',course.rating],['Conditions',course.conditions],['Value',course.value],['Vibes',course.vibes]].map(([label,val]) => (
            <div key={label} style={{ textAlign:'center' }}>
              <div style={{ color:B.gold, fontSize:22, fontWeight:900, fontFamily:serif, lineHeight:1 }}>{val.toFixed(1)}</div>
      ...
      {tab==='feed' && (
        <div>
          <button style={{ width:'100%', background:B.gold, color:B.navy, border:'none', borderRadius:12, padding:'14px 0', fontWeight:800, fontSize:15, cursor:'pointer', marginBottom:14, fontFamily:serif }}>+ Log Your Round Here</button>
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            {show.map((item,i) => (
              <div key={i} style={{ background:'#fff', borderRadius:14, border:`1px solid ${B.border}`, padding:'16px 18px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
                  <Avatar initials={item.avatar} size={40} color={i%2===0 ? B.navy:B.green}/>
                  <div style={{ flex:1 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:10, borderTop:`1px solid ${B.feedBg}` }}>
                  ...
                  <button style={{ background:'none', border:'none', color:B.textSoft, cursor:'pointer', fontSize:13, fontFamily:sans }}>♡ {item.likes}</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {tab==='ratings' && (
        <div style={{ background:'#fff', borderRadius:16, padding:24, border:`1px solid ${B.border}` }}>
          <div style={{ textAlign:'center', marginBottom:24 }}>
            <div style={{ fontSize:52, fontWeight:900, color:B.gold, fontFamily:serif, lineHeight:1 }}>{course.rating}</div>
            <div style={{ fontSize:13, color:B.textSoft, fontFamily:sans, marginTop:4 }}>Based on {course.reviews.toLocaleString()} ratings</div>
          </div>
